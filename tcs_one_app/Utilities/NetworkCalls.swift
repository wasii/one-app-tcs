...
//                        handler(true, data)
    ...
    //MARK: RIDER DELIVERY SHEET
    class func getriderdeliverysheets(params: [String:Any], handler: @escaping(_ granted: Bool,_ response: Any) -> Void) {
        let Url = String(format: RIDER_DELIVERY_SHEET)
        guard let serviceUrl = URL(string: Url) else { return }
        var request = URLRequest(url: serviceUrl)
        request.httpMethod = "POST"
        request.setValue("Bearer \(RIDER_BEARER_TOKEN)", forHTTPHeaderField: "Authorization")
    ...
    //MARK: RIDER BIN INFO
    class func getriderbininfo(params: [String:Any], handler: @escaping(_ granted: Bool,_ response: Any) -> Void) {
        let Url = String(format: RIDER_BIN_INFO)
        guard let serviceUrl = URL(string: Url) else { return }
        var request = URLRequest(url: serviceUrl)
        request.httpMethod = "POST"
        request.setValue("Bearer \(RIDER_BEARER_TOKEN)", forHTTPHeaderField: "Authorization")
    ...
    //MARK: - Rider Request Dispute
    class func postriderrequestdispute(params: [String:Any], handler: @escaping(_ granted: Bool,_ response: Any) -> Void) {
        let Url = String(format: RIDER_REQUEST_DISPUTE)
        guard let serviceUrl = URL(string: Url) else { return }
        var request = URLRequest(url: serviceUrl)
        request.httpMethod = "POST"
        request.setValue("Bearer \(RIDER_BEARER_TOKEN)", forHTTPHeaderField: "Authorization")
    ...
    //MARK: - Rider Request Dispute
    class func getriderqrcode(params: [String:Any], handler: @escaping(_ granted: Bool,_ response: Any) -> Void) {
        let Url = String(format: RIDER_GENERATE_GIVENTO)
        guard let serviceUrl = URL(string: Url) else { return }
        var request = URLRequest(url: serviceUrl)
        request.httpMethod = "POST"
        request.setValue("Bearer \(RIDER_BEARER_TOKEN)", forHTTPHeaderField: "Authorization")
        ...
        session.dataTask(with: request) { (data, response, error) in
            if let data = data {
                ...
                if let success = json.dictionary?[returnStatus] {
                    //SUCCESS
                    if success.dictionary?[_code] == "0200" {
                        if let qrCode = json.dictionary?[_qrCode]?.string {
                            handler(true, qrCode)
                            return
                        } else {
                            handler(false, "QR CODE NOT VALID")
                        }
                    }
                    //FAILED
                    if success.dictionary?[_code] == "0400" {
    ...
    //MARK: - RIDER TAKE OVER
    class func postriderqrcode(params: [String:Any], handler: @escaping(_ granted: Bool,_ response: Any) -> Void) {
        ...
        session.dataTask(with: request) { (data, response, error) in
            if let data = data {
                ...
                if let success = json.dictionary?[returnStatus] {
                    //SUCCESS
                    if success.dictionary?[_code] == "0200" {
                        if let deliverysheet = json.dictionary?[_deliverysheet]?.string {
                            handler(true, deliverysheet)
                            return
                        } else {
                            handler(false, "deliverysheet NOT VALID")
                        }
                    }
                    //FAILED
                    if success.dictionary?[_code] == "0400" {
