//
//  HRFilesViewController.swift
//  tcs_one_app
//
//  Created by TCS on 15/01/2021.
...
class HRFilesViewController: BaseViewController {
    ...
    func setupAttachments(_ handler: @escaping(_ success: Bool, _ count: Int)-> Void) {
        ...
        if let ticket = AppDelegate.sharedInstance.db?.read_tbl_hr_request(ticketId: self.ticket_id!) {
            if ticket.first?.RESPONSIBLE_EMPNO == Int(CURRENT_USER_LOGGED_IN_ID) {
                handler(true, fileAttachments?.count ?? 0)
            } else {
...
extension HRFilesViewController: UITableViewDelegate, UITableViewDataSource {
...
extension HRFilesViewController: QLPreviewControllerDataSource {
