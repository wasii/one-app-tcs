...
// MARK: - Welcome
struct User: Codable {
//    let empid = data.array?.first?["EMPID"].int ?? 0
//    let empname = data.array?.first?["EMP_NAME"].string ?? ""
//    let gender = data.array?.first?["GENDER"].string ?? ""
//    let cnicNo = data.array?.first?["CNIC_NO"].string ?? ""
//    let disableStatus = data.array?.first?["DISABLE_STATUS"].string ?? ""
//    let currCity = data.array?.first?["CURR_CITY"].string ?? ""
//    let empCell1 = data.array?.first?["EMP_CELL#1"].string ?? ""
//    let empCell2 = data.array?.first?["EMP_CELL#2"].string ?? ""
//    let gradecode = data.array?.first?["GRADE_CODE"].string ?? ""
//    let empstatus = data.array?.first?["EMP_STATUS"].string ?? ""
//    let unitcode = data.array?.first?["UNIT_CODE"].string ?? ""
//    let workingDesig = data.array?.first?["WORKING_DESIG"].string ?? ""
//    let deptCode = data.array?.first?["DEPT_CODE"].string ?? ""
//    let subDeptCode = data.array?.first?["SUB_DEPT_CODE"].string ?? ""
//    let userid = data.array?.first?["USERID"].string ?? ""
//    let areaCode = data.array?.first?["AREA_CODE"].string ?? ""
//    let stationCode = data.array?.first?["STATION_CODE"].string ?? ""
//    let hubCode = data.array?.first?["HUB_CODE"].string ?? ""
//    let highness = data.array?.first?["HIGHNESS"].string ?? ""
    let empid: Int?
    let empName, gender, cnicNo, disableStatus: String?
    let currCity, empCell1, empCell2: String?
...
struct AttLocations: Codable {
    ...
    let locCode, hubCode, locName, latitude, longitude: String
    let radius: Int
